...
class InputHandler:
    ...
    def update_torch_export_outputs(self, torch_outputs):
        """
        Function responsible for updating torch.export model outputs.

        ``Mandatory`` Args:
            :torch_outputs (tuple): PyTorch outputs from torch.export ExportedProgram

        Return:
            :Dict: Updated past_key_values and logits
        """
        outputs = {}
        outputs["past_key_values"] = torch_outputs.past_key_values
        outputs["logits"] = torch_outputs.logits

        return outputs

    def update_torch_export_inputs(self, inputs, torch_outputs):
        """
        Function responsible for updating inputs for next iteration with torch.export outputs.

        ``Mandatory`` Args:
            :inputs (Dict): PyTorch inputs from previous iteration
            :torch_outputs (Dict): PyTorch outputs from previous iteration
        ...
        # Extract the next token from logits
        updated_inputs["input_ids"] = torch_outputs["logits"].argmax(-1)
        
        # Update position_ids for next iteration
        updated_inputs["position_ids"] = torch.max(inputs["position_ids"], dim=1, keepdim=True)[0] + 1
        
        # Update past_key_values for next iteration
        # torch.export expects a nested list [[key, value], ...]
        past_key_values = torch_outputs["past_key_values"]
        if isinstance(past_key_values, (list, tuple)) and len(past_key_values) > 0:
            updated_inputs["past_key_values"] = [
                [kv[0], kv[1]] if isinstance(kv, (list, tuple)) else kv for kv in past_key_values
            ]
